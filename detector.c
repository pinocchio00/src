...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
	...
	/*here my code*/
        ...
        printf("%d: loss: %f, avg_loss: %f, %f rate, %lf seconds, %d images\n", get_current_batch(net), loss, avg_loss, get_current_rate(net), sec(clock()-time), i*imgs);
	/*here my code before my change, i%100 == 0*/
        if(i%1000==0 || (i < 1000 && i%5 == 0)){
#ifdef GPU
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh,int image_num)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
	...
	/*here my code*/
	char image_name[30] = "result/";
	strncat(image_name,filename,strlen(filename)-4);
        save_image(im, image_name);
        show_image(im, image_name);
	/**/
...
void run_detector(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh,0);
