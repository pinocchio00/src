...
void forward_region_layer(const layer l, network_state state)
{
    ...
    *(l.cost) = 0;
    ...
    //printf("%d\n",b);
        ...
        for(t = 0; t < 30; ++t){
            box truth = float_to_box(state.truth + t*5 + b*l.truths);
            // here my code
	    //printf("before truth box.x = %f \n",truth.x);
	    //printf("state.truth [0]=%f, [1]=%f, [2]=%f,[3]=%f\n",state.truth[0],state.truth[1],state.truth[2],state.truth[3]);
	    //printf("t=%d, b=%d, l.truths=%d\n",t,b,l.truths);
	    //printf("t*5+b*l.truths=%d\n",t*5 + b*l.truths);
            if(!truth.x) break;
	    //printf("after truth\n");
            float best_iou = 0;
            int best_index = 0;
    ...
    *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
    ...
    /*here my code*/
    printf("Region Avg IOU: %f,avg cat: %f ,Class_count: %d, Obj: %f, No Obj: %f, Recall: %f,  count: %d\n", avg_iou, avg_cat,class_count, avg_obj, avg_anyobj/(l.w*l.h*l.n*l.batch), recall, count);
}
